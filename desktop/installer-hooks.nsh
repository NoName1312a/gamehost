; Custom NSIS hooks for the GameNest installer.
;
; The engine and frpc tunnel client run as background "sidecar" processes. When
; an update is installed while GameNest is running, those processes keep
; engine.exe / frpc.exe open and the installer fails with
...
!macro NSIS_HOOK_PREINSTALL
  ...
  nsExec::Exec 'taskkill /F /IM frpc.exe'
!macroend
...
!macro NSIS_HOOK_PREUNINSTALL
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION "Also remove all GameNest servers, their saved worlds, and Docker volumes?$\n$\nChoose No to keep them for a future reinstall." /SD IDNO IDNO gh_skip_purge
